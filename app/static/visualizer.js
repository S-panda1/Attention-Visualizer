class StaticGridVisualization {
    ...
    renderVisualization() {
        ...
        console.log("Tokens:", this.data?.tokens?.length, this.data.tokens);
        console.log("Layers:", this.data?.layers?.length, this.data.layers);

        if (!this.data || !this.data.layers || this.data.layers.length === 0) {
            console.error("No visualization data available — payload:", this.data);
            // show in-page message
            const container = document.getElementById('visualization');
            container.innerHTML = "<p style='color:lightcoral; text-align:center;'>No data to visualize.<br/>Check console for details.</p>";
            return;
        }
...
function initializeGridVisualization(data) {
    ...
    // … end of StaticGridVisualization class …

    // make your class available globally
    window.initializeGridVisualization = function(data) {
        // this must match the <div id="visualization"> in index.html
        const vis = new StaticGridVisualization('visualization', {
            threshold: parseFloat(document.getElementById('threshold-slider').value),
            tokenSpacing: 55,
            // any other config overrides…
        });
        vis.data = data;
        vis.renderVisualization();
    };

    // alias (so you can call initializeGridVisualization(data) directly)
    function initializeGridVisualization(data) {
        window.initializeGridVisualization(data);
    }
